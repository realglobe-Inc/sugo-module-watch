...
const WatchModule = require('../lib/watch_module.js')
...
describe('demo-module', function () {
  ...
  it('Get module spec', () => co(function * () {
    let module = new WatchModule({ $emitter: new EventEmitter() })
    assert.ok(module)
  ...
  it('Compare methods with spec', () => co(function * () {
    let module = new WatchModule({ $emitter: new EventEmitter() })
    let { $spec } = module
    let implemented = Object.getOwnPropertyNames(WatchModule.prototype)
